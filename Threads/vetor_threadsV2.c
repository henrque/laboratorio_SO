...
int main(){
	...
	for(int i=0; i<N; i++){
		...
		printf("Não foi encontrado na thread: %d\n",dados[i].numThread);
		}
		else{
			printf("Foram encontrados %d vezes na thread: %d \n",dados[i].flagEncontrou,dados[i].numThread);
		}
	}
	/*
	for(int i = 0; i<V; i++){
